...
### 密钥失效降级（fallback_on_auth）

默认情况下，上游返回 401/403 会被当成**调用方的问题**（模型名写错、客户端
key 不对），直接把 401 甩回给调用方——换渠道重试只是白烧下一个渠道的配额。

但对「一条路由挂了多把 provider key」的场景，401 的含义不同：**这个渠道的
key 坏了**，正是备用 key 存在的意义。在控制台把这条路由的「密钥失效降级」
打开后，渠道的 401/403 会加入可转移集合（与 5xx / 429 / 408 并列），自动换
下一个渠道重试。

三个刻意的边界：

- **失败照样进熔断计数**——坏 key 的渠道几次之内就被熔断踢出候选，
  健康探测恢复后再放回来。转移处理头几次失败，熔断处理剩下的。
- **最后一个渠道的 401 原样返回**——没有下一个可转移时，真实的上游回答
  不能被包成 502，否则"所有 key 都死了"会被读成"网关挂了"。
- **默认关**——模型名写错导致的 401 不该换渠道烧配额。

典型用法：K3 路由挂主 key（优先级 0）+ 备用 key（优先级 1），打开降级开关。
主 key 失效时自动落备用，对健康探测与熔断全透明。
