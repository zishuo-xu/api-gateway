...
- 5xx、429（上游限流）、408（上游超时）自动换渠道重试，其他 4xx 不重试
...
**为什么只有 5xx / 429 / 408 重试**
判据是失败原因属于「请求错了」还是「这条路暂时走不通」。
400 / 401 / 403 / 404 是请求或凭证本身的问题，换渠道会得到同样的结果，
重试只是浪费上游额度和客户端时间。429 是上游限流、408 是上游超时，
换一条暂时通畅的路可能就成了，所以归入可重试。详见 `docs/design.md` 第 4 节。
